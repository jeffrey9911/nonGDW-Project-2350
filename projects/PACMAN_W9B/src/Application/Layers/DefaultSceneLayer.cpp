...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		Texture2D::Sptr		terrianTex = ResourceManager::CreateAsset<Texture2D>("objs/terrain/GroundTex.png");
		Texture2D::Sptr		goalTex = ResourceManager::CreateAsset<Texture2D>("objs/coin/CoinUV.png");
		Texture2D::Sptr		rockTex = ResourceManager::CreateAsset<Texture2D>("objs/rocks/rockTex.png");
		Texture2D::Sptr		branchTex = ResourceManager::CreateAsset<Texture2D>("objs/branches/branchTex.png");
...
#pragma endregion 
		...
		Material::Sptr terrianMaterial = ResourceManager::CreateAsset<Material>(deferredShader);
		{
		...
		Material::Sptr carTextureMat = ResourceManager::CreateAsset<Material>(deferredShader);
		{
		...
		Material::Sptr goalTextureMat = ResourceManager::CreateAsset<Material>(deferredShader);
		{
		...
		Material::Sptr rockMat = ResourceManager::CreateAsset<Material>(deferredShader);
		{
		...
		Material::Sptr branchMat = ResourceManager::CreateAsset<Material>(deferredShader);
		{
			rockMat->Name = "Rock-Mat";
			rockMat->Set("u_Material.Diffuse", branchTex);
			rockMat->Set("u_Material.Shininess", 1.0f);
		}
		...
		DuoPlayerCamera::Sptr dCmr = camera->Add<DuoPlayerCamera>();
		...
		{
			rock1->SetPostion(glm::vec3(-2.0f, -5.0f, 0.0f));
		...
		/*
		GameObject::Sptr plane = scene->CreateGameObject("Plane - Specular Demo");
		{
		...
		GameObject::Sptr chara1 = scene->CreateGameObject("PACMAN");
		{
			chara1->SetPostion(glm::vec3(-3.0f, 3.0f, 3.0f));
			chara1->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			RenderComponent::Sptr renderer = chara1->Add<RenderComponent>();
			renderer->SetMesh(characterMesh);
			renderer->SetMaterial(characterTextureMat);


			SimpleObjectControl::Sptr objControl = chara1->Add<SimpleObjectControl>();
			objControl->setCamera(camera);
			objControl->playerInd = 1;

			//scene->MainCamera->GetGameObject()->Get<SimpleCameraFollow>()->setFollowObj(chara1);
			//scene->MainCamera->GetGameObject()->Get<DuoPlayerCamera>()->player1 = chara1;
			dCmr->player1 = chara1;

			RigidBody::Sptr physics = chara1->Add<RigidBody>(RigidBodyType::Dynamic);
			...
			SimpleToggle::Sptr toggle = chara1->Add<SimpleToggle>();
			toggle->_shaderContainer = shaderContainer;
			toggle->toggleMode = 2;
		}

		GameObject::Sptr chara2 = scene->CreateGameObject("PACMAN");
		{
			chara2->SetPostion(glm::vec3(3.0f, -3.0f, 3.0f));
			chara2->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			RenderComponent::Sptr renderer = chara2->Add<RenderComponent>();
			renderer->SetMesh(characterMesh);
			renderer->SetMaterial(characterTextureMat);


			SimpleObjectControl::Sptr objControl = chara2->Add<SimpleObjectControl>();
			objControl->setCamera(camera);
			objControl->playerInd = 2;

			dCmr->player2 = chara2;
			//scene->MainCamera->GetGameObject()->Get<SimpleCameraFollow>()->setFollowObj(chara2);
			//scene->MainCamera->GetGameObject()->Get<DuoPlayerCamera>()->player1 = chara2;


			RigidBody::Sptr physics = chara2->Add<RigidBody>(RigidBodyType::Dynamic);
			ICollider::Sptr collider = physics->AddCollider(SphereCollider::Create());
			//collider->SetPosition(glm::vec3(0.0f, 0.0f, 0.0f));
			//collider->SetScale(glm::vec3(0.5f, 0.5f, 0.5f));
			physics->AddComponent<TriggerVolume>();
			...
			SimpleToggle::Sptr toggle = chara2->Add<SimpleToggle>();
			toggle->_shaderContainer = shaderContainer;
			toggle->toggleMode = 2;
		}
