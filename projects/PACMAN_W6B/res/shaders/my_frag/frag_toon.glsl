...
void main() {
	...
	vec3 lightAccumulation = CalcAllLightContribution(inViewPos, normal, u_CamPos.xyz, specPower);

	// Get the albedo from the diffuse / albedo map
	vec4 textureColor = texture(u_Material.Diffuse, inUV);

	textureColor.r = texture(toonTex, textureColor.r).r;
	textureColor.g = texture(toonTex, textureColor.g).g;
	textureColor.b = texture(toonTex, textureColor.b).b;
